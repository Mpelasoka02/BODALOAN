        ...
        <div class="hero-visual">
            <div class="hero-card">
                ...
                <div class="hero-stat">
                    ...
                    <div>
                        <div class="hero-stat-num">{{ \App\Models\Motorcycle::where('verification_status','verified')->where('status','available')->count() }}+</div>
                        <div class="hero-stat-label">Verified BodaBodas</div>
                    </div>
                </div>
                <div class="hero-mini">
                    <div class="hero-mini-card">
                        <div class="num">{{ \App\Models\User::where('role','driver')->count() }}+</div>
                        <div class="lbl">Active Drivers</div>
                    </div>
                    <div class="hero-mini-card">
                        <div class="num">{{ \App\Models\Loan::where('status','active')->count() }}+</div>
...
<section class="section bg-gray" id="browse">
    ...
    <div class="filters">
        <a href="{{ route('home') }}" class="pill {{ !request('min_price') && !request('max_price') ? 'active' : '' }}">All</a>
        <a href="{{ route('home', array_merge(request()->query(), ['max_price' => 500000])) }}" class="pill {{ request('max_price') == 500000 ? 'active' : '' }}">Under 500K</a>
        <a href="{{ route('home', array_merge(request()->query(), ['min_price' => 500000, 'max_price' => 1500000])) }}" class="pill {{ request('min_price') == 500000 ? 'active' : '' }}">500K – 1.5M</a>
        <a href="{{ route('home', array_merge(request()->query(), ['min_price' => 1500000, 'max_price' => 2500000])) }}" class="pill {{ request('min_price') == 1500000 ? 'active' : '' }}">1.5M – 2.5M</a>
        <a href="{{ route('home', array_merge(request()->query(), ['min_price' => 2500000])) }}" class="pill {{ request('min_price') == 2500000 ? 'active' : '' }}">2.5M+</a>
        <span class="results-text"><strong>{{ $motorcycles->total() }}</strong> boda bodas</span>
        <select class="sort-select" onchange="window.location.href=this.value">
            <option value="{{ route('home', array_merge(request()->query(), ['sort' => 'newest'])) }}" {{ request('sort', 'newest') == 'newest' ? 'selected' : '' }}>Newest</option>
            <option value="{{ route('home', array_merge(request()->query(), ['sort' => 'price_low'])) }}" {{ request('sort') == 'price_low' ? 'selected' : '' }}>Price: Low to High</option>
            <option value="{{ route('home', array_merge(request()->query(), ['sort' => 'price_high'])) }}" {{ request('sort') == 'price_high' ? 'selected' : '' }}>Price: High to Low</option>
        </select>
    </div>
    ...
    @if($motorcycles->count() > 0)
        <div class="grid">
            @foreach($motorcycles as $m)
                <a href="{{ route('marketplace.show', $m) }}" class="card">
                    <div class="card-img">
                        @if($m->photo)
                        ...
                        @endif
                        @if($m->engine_cc)<div class="card-cc">{{ $m->engine_cc }}cc</div>@endif
                    </div>
                    <div class="card-body">
                        <div class="card-name">{{ $m->make }} {{ $m->model }}</div>
                        <div class="card-detail">{{ $m->plate_number }}</div>
                        @if($m->loan_amount)
                        ...
                        <div class="card-specs">
                            <div class="card-spec"><i class="bi bi-calendar-week"></i> {{ $m->loan_duration_weeks }} weeks</div>
                            <div class="card-spec"><i class="bi bi-cash"></i> TZS {{ $m->weekly_amount ? number_format($m->weekly_amount) : '—' }}/wk</div>
                        </div>
                    </div>
                </a>
            @endforeach
        </div>
        <div class="pager">{{ $motorcycles->withQueryString()->links() }}</div>
    @else
        <div style="text-align:center;padding:64px 20px">
            ...
            <a href="{{ route('home') }}" style="display:inline-flex;align-items:center;gap:6px;padding:10px 22px;border-radius:10px;background:#1B3358;color:#fff;text-decoration:none;font-weight:600;font-size:.88rem;transition:all .2s">Clear Filters</a>
        </div>
    @endif
</section>
