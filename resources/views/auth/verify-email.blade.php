...
<head>
    ...
    <title>Verify Email — BodaLink</title>
...
<body>
    ...
    <div class="auth-card">
        ...
        <div class="subtitle">Enter the 6-digit code we sent to your email</div>

        @if(session('success'))
            <div class="alert-success"><i class="bi bi-check-circle me-1"></i> {{ session('success') }}</div>
        @endif
        @if(session('error'))
            <div class="alert-error"><i class="bi bi-exclamation-circle me-1"></i> {{ session('error') }}</div>
        @endif
        @if($errors->any())
        ...
        <form method="POST" action="{{ route('verify.email.submit') }}">
            ...
            <div class="field">
                ...
                <div class="field-input">
                    <input type="text" name="code" maxlength="6" inputmode="numeric" pattern="[0-9]{6}" autocomplete="off" autofocus placeholder="000000">
                </div>
            </div>
